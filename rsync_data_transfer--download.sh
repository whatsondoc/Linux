...
if [[ ${PROCS} -gt $(ssh ${USER}@${REMOTE_HOST} nproc) ]]
then
...
if ! ssh ${USER}@${REMOTE_HOST} [[ -x $(command -v taskset) ]]
then 
	echo -e "\nERROR:\t\ttaskset is not present on the remote server (or, at least, not included in '$PATH'). It is typically available in the util-linux package in Linux.\n"
	TERMINATE="true"
fi
...
in
    d) LOCAL_DIR=${OPTARG}														## The directory specified by the user from which to transfer files, parsed from the input value in the script argument
		...
		then 
			...
			echo -e "\nERROR:\tPlease specify a valid directory to which the files will be copied.\n"
			help
			exit 1
		fi
	;;
	...
	p) PROCS=${OPTARG}
		...
		then
			echo -e "VALIDATED:\tAll server processors selected."
			PROCS=$(nproc)
...
TOTAL_TASKS=$(ssh ${USER}@${REMOTE_HOST} find ${REMOTE_DIR} -type f | wc -l)    ## The total number of files in the supplied directory path to be transferred
FILE_QUEUE=( $(ssh ${USER}@${REMOTE_HOST} ls ${REMOTE_DIR}) )					## Creating a variable array that contains the file names that are to be transferred
...
then
    ...
    do 
        ssh ${USER}@${REMOTE_HOST} echo "${FILE_CHECKSUM} `sha1sum ${REMOTE_DIR}/${FILE_QUEUE[${FILE_CHECKSUM_INDEX}]} | awk '{print $1}'`"  >> /dev/shm/data-transfer-file-checksum.remote
        ((FILE_CHECKSUM_INDEX++))
    done
    echo -e "Complete.\n"
fi
...
do
    ...
    do
		...
		## The exit path, for when the FILE_INDEX counter exceeds the value in TOTAL_TASKS:
			...
			until [[ $(ssh ${USER}@${REMOTE_HOST} /usr/sbin/pidof rsync | wc -w) == 0 ]]
			do
			...
			echo -e "\vOPERATION COMPLETE: Submitted ${FILE_INDEX} files `if [[ -n ${DATA_TRANSFER_COUNT} ]]; then echo "at ${DATA_TRANSFER_COUNT} "; fi`for transfer from ${REMOTE_HOST}:${REMOTE_DIR} to ${LOCAL_DIR}\v"
